...
#include <sys/cdefs.h>
__FBSDID("$FreeBSD: head/sys/dev/iwn/if_iwn.c 243857 2012-12-04 09:32:43Z glebius $");
...
static int 	iwn_newstate1(struct ieee80211vap *vap, enum ieee80211_state nstate, int arg);
...
static int 	iwn_set_timing1(struct iwn_softc *sc);
...
static int	iwn_config1(struct iwn_softc *);
...
static int	iwn_auth1(struct iwn_softc *, struct ieee80211vap *vap);
static int	iwn_run(struct iwn_softc *, struct ieee80211vap *vap);
static int	iwn_run1(struct iwn_softc *, struct ieee80211vap *vap);
    ...
    const uint8_t mac[IEEE80211_ADDR_LEN])
{
	...
	//ieee80211_ratectl_init(vap); xxx: not sure if this should be removed (as i_w_l suggests)
...
static int
iwn_updateedca1(struct ieee80211com *ic)
{
#define IWN_EXP2(x)     ((1 << (x)) - 1)        /* CWmin = 2^ECWmin - 1 */
        struct iwn_softc *sc = ic->ic_ifp->if_softc;
        struct iwn_edca_params cmd;
...
static int
iwn_set_timing1(struct iwn_softc *sc)
{
        struct iwn_cmd_timing cmd;
        int error = 0;
        struct ieee80211vap *vap;
...
iwn5000_get_temperature(struct iwn_softc *sc)
{
	...
	return temp; //xxx: i_w_l suggests return type to be void but keeping it int
			// to be in sync with other iwn*_get_temperature() functions.
}
...
static int
iwn_config1(struct iwn_softc *sc)
{
        struct ifnet *ifp = sc->sc_ifp;
        struct ieee80211com *ic = ifp->if_l2com;
        uint16_t rxchain;
...
static int
iwn_run1(struct iwn_softc *sc, struct ieee80211vap *vap)
{
        struct iwn_ops *ops = &sc->ops;
        struct ifnet *ifp = sc->sc_ifp;
        struct ieee80211com *ic = ifp->if_l2com;
        ...
        if ((error = iwn_updateedca1(ic)) != 0) {
                device_printf(sc->sc_dev,
                    "%s: iwn_updateedca1, error %d\n", __func__, error);
                return error;
        }
    ...
    uint8_t tid)
{
	...
	//xxx: why remove this? - iwn_set_link_quality(sc, ni);
	return 1;
}
